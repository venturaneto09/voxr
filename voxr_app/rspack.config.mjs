...
export default () => {
	...
	const staticCdnEndpoint = envString(
		...
		envString('VOXR_STATIC_CDN_ENDPOINT', isProduction ? '' : ''),
	);
	...
	return {
		...
		resolve: {
			alias: {
				...
				'@app': SRC_DIR,
				'@voxr/voice_engine_v2/bridge': path.join(
					MONOREPO_ROOT,
					'packages',
					'voice_engine_v2',
					'src',
				...
				'@voxr/voice_engine_v2/runtime': path.join(
					MONOREPO_ROOT,
					'packages',
					'voice_engine_v2',
					'src',
				...
				'@voxr/voice_engine_v2/testing': path.join(
					MONOREPO_ROOT,
					'packages',
					'voice_engine_v2',
					'src',
				...
				'@voxr': path.join(MONOREPO_ROOT, 'packages'),
				'@pkgs': PKGS_DIR,
		...
		plugins: [
			...
			new DefinePlugin({
				__VOXR_PRECACHE_MANIFEST__: JSON.stringify([]),
				__VOXR_SW_VERSION__: JSON.stringify(publicValues.PUBLIC_BUILD_VERSION || 'dev'),
		...
		devServer: {
			port: Number(process.env.VOXR_APP_DEV_PORT) || DEFAULT_DEV_SERVER_PORT,
			hot: false,
			liveReload: false,
			client: false,
