...
(function () {
	...
	var key = 'voxr-admin-sidebar-scroll';
	var restored = false;
	try {
...
(function () {
	if (window.__voxrAdminActionForms) return;
	window.__voxrAdminActionForms = true;
...
(function () {
	if (window.__voxrAdminHtmxScrollPreserver) return;
	window.__voxrAdminHtmxScrollPreserver = true;
	var pendingKey = '__voxrAdminPendingHtmxScroll';
	var restoreTimer = 0;
...
(function () {
	if (window.__voxrAdminToastScript) return;
	window.__voxrAdminToastScript = true;
	...
	function markToastHandled(detail) {
		if (detail) detail.__voxrToastHandled = true;
		if (detail && detail.xhr) detail.xhr.__voxrToastHandled = true;
	}

	function toastWasHandled(detail) {
		return !!(detail && (detail.__voxrToastHandled || (detail.xhr && detail.xhr.__voxrToastHandled)));
	}
	...
	function disableMutationControls(detail) {
		...
		mutationControls(detail).forEach(function (control) {
			if (control.disabled || control.dataset.voxrAdminRequestDisabled === 'true') return;
			control.disabled = true;
			control.dataset.voxrAdminRequestDisabled = 'true';
			disabled.push(control);
		});
		if (detail) detail.__voxrDisabledControls = disabled;
		if (detail && detail.xhr) detail.xhr.__voxrDisabledControls = disabled;
	}

	function restoreMutationControls(detail) {
		var controls = (detail && detail.__voxrDisabledControls) ||
			(detail && detail.xhr && detail.xhr.__voxrDisabledControls) ||
			[];
		controls.forEach(function (control) {
			if (control.dataset.voxrAdminRequestDisabled !== 'true') return;
			control.disabled = false;
			delete control.dataset.voxrAdminRequestDisabled;
		});
	}
	...
	function parseAdminToastHeader(xhr) {
		...
		var raw = xhr.getResponseHeader('X-Voxr-Admin-Toast');
		if (!raw) return null;
		try {
