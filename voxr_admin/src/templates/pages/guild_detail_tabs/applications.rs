                ...
                @if applications.is_empty() {
                    div class="flex flex-col items-center justify-center py-12 text-center" {
                        ...
                        p class="mt-1 text-sm text-neutral-500" {
                            "This guild does not have any installed applications."
                        }
                    }
                } @else {
                    div class="space-y-4" {
            ...
            div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between" {
                div class="min-w-0 flex-1 space-y-2" {
                    div class="flex flex-wrap items-center gap-2" {
                        ...
                        @if app.bot_user_id.is_some() {
                            span class="rounded bg-blue-100 px-2 py-0.5 text-xs font-medium \
                                        uppercase text-blue-700" {
                                "Bot"
                            }
                        } @else {
                            span class="rounded bg-neutral-200 px-2 py-0.5 text-xs font-medium \
                ...
                a href={(base) "/applications/" (app.id)}
                    class="inline-flex items-center rounded-md bg-brand-primary px-4 py-2 \
                           text-sm font-medium text-white shadow-sm hover:bg-brand-primary-dark" {
                    "View Details"
                }
            }
        }
