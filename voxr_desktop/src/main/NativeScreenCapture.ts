...
function loadMacNativeScreenCaptureAddon(): NativeAddonLoadResult {
	try {
		const addon = requireModule('@voxr/mac-screen-capture') as MacNativeScreenCaptureModule;
		if (addon.loadError) {
...
function loadLinuxNativeScreenCaptureAddon(): NativeAddonLoadResult {
	try {
		const addon = requireModule('@voxr/linux-screen-capture') as LinuxNativeScreenCaptureModule;
		if (addon.loadError) {
			const detail = describeAddonLoadError(addon.loadError);
...
function loadWindowsNativeScreenCaptureAddon(): NativeAddonLoadResult {
	try {
		const addon = requireModule('@voxr/win-game-capture') as WindowsNativeScreenCaptureModule;
		if (addon.loadError) {
			const detail = describeAddonLoadError(addon.loadError);
