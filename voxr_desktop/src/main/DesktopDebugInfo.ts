...
const DEBUG_INFO_ARGS = new Set(['--voxr-debug-info', '--voxr-client-info', '--client-info']);
const DISABLE_HARDWARE_ACCELERATION_ARGS = new Set(['--voxr-disable-gpu', '--voxr-disable-hardware-acceleration']);
const OPEN_DEVTOOLS_ARGS = new Set(['--voxr-devtools', '--voxr-open-devtools']);
const RESET_WINDOW_STATE_ARGS = new Set(['--voxr-reset-window-state']);
const SAFE_MODE_ARGS = new Set(['--voxr-safe-mode']);
const RENDERER_CONSOLE_LOG_ARGS = new Set(['--voxr-log-renderer-console']);
const NET_LOG_ARGS = new Set(['--voxr-net-log']);
const APP_URL_ARGS = new Set(['--voxr-app-url']);
...
export function getLaunchAppUrlOverride(argv: ReadonlyArray<string>): string | null {
	...
	if (!trimmed) {
		throw new Error('--voxr-app-url requires a URL');
	}
	const candidate = /^[a-zA-Z][a-zA-Z0-9+\-.]*:\/\//.test(trimmed) ? trimmed : `https://${trimmed}`;
	const url = new URL(candidate);
	if (url.protocol !== 'https:' && url.protocol !== 'http:') {
		throw new Error('--voxr-app-url must use http or https');
	}
	return url.toString();
}
...
function sanitizeLaunchArg(arg: string): string {
	if (/^voxr:\/\//i.test(arg)) {
		return 'voxr://<redacted>';
	}
	if (arg.startsWith('--voxr-app-url=')) {
		return `--voxr-app-url=${sanitizeUrlForDiagnostics(arg.slice('--voxr-app-url='.length))}`;
	}
