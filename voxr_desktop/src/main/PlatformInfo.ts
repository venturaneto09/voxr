...
function loadMacSysctl(): MacSysctlModule | null {
	...
	try {
		macSysctlCache = requireModule('@voxr/mac-sysctl') as MacSysctlModule;
	} catch (error) {
		throw new Error(
			`@voxr/mac-sysctl failed to load on macOS; native diagnostics cannot continue. Original error: ${
...
function loadNativePlatformInfo(): NativePlatformInfoModule | null {
	...
	try {
		nativePlatformInfoCache = requireModule('@voxr/platform-info') as NativePlatformInfoModule;
	} catch {
