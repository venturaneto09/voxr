...
mod tests {
    ...
    fn valid_raw_options() -> CreateShortcutOptions {
        CreateShortcutOptions {
            lnk_path: Some("C:\\Users\\Example\\Desktop\\Voxr.lnk".to_string()),
            target: Some("C:\\Program Files\\Voxr\\Voxr.exe".to_string()),
            args: None,
    ...
    fn valid_run_value_options() -> SetRunValueOptions {
        SetRunValueOptions {
            name: Some("velopack.voxr_desktop_canary".to_string()),
            command: Some(
                "\"C:\\Users\\Example\\AppData\\Local\\Voxr\\Voxr.exe\" --autostart"
                    .to_string(),
            ),
        }
    }
    ...
    fn validation_preserves_optional_fields() {
        ...
        raw.icon_path = Some("C:\\Program Files\\Voxr\\Voxr.exe".to_string());
        raw.icon_index = Some(2);
        raw.working_dir = Some("C:\\Program Files\\Voxr".to_string());
        raw.description = Some("Voxr desktop client".to_string());
        raw.app_user_model_id = Some("velopack.voxr_desktop".to_string());
        ...
        assert_eq!(
            ...
            Some("C:\\Program Files\\Voxr\\Voxr.exe")
        );
        assert_eq!(opts.icon_index, 2);
        assert_eq!(
            opts.working_dir.as_deref(),
            Some("C:\\Program Files\\Voxr")
        );
        assert_eq!(opts.description.as_deref(), Some("Voxr desktop client"));
    }
    ...
    fn run_value_validation_preserves_fields() {
        ...
        assert_eq!(opts.name, "velopack.voxr_desktop_canary");
        assert_eq!(
            opts.command,
            "\"C:\\Users\\Example\\AppData\\Local\\Voxr\\Voxr.exe\" --autostart"
        );
