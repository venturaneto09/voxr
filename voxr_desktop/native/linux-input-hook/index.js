...
const MODULE_NAME = '@voxr/linux-input-hook';
const SKIP_NATIVE_PROBE_ENV = 'VOXR_LINUX_INPUT_HOOK_SKIP_NATIVE_PROBE';
...
function nativeFileName() {
	if (process.platform !== 'linux') {
		throw new Error(`@voxr/linux-input-hook is only supported on Linux, got ${process.platform}`);
	}
	switch (process.arch) {
		case 'x64':
