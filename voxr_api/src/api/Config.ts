...
function normalizeIpBanExemptIps(values: Array<string>): Array<string> {
	...
	for (const value of values) {
		...
		if (!parsed) {
			throw new Error(`VOXR_API_IP_BAN_EXEMPT_IPS contains an invalid IP address: ${value}`);
		}
...
export function buildAPIConfigFromMaster(master: MasterConfig): APIConfig {
	...
	if (uploadRelaySecretBase64.length === 0) {
		throw new Error('VOXR_MEDIA_PROXY_UPLOAD_RELAY_SECRET_BASE64 is required for the API');
	}
	...
	return {
		...
		postgres: {
			...
			database: postgresSource?.database ?? 'voxr',
			username: postgresSource?.username ?? 'voxr',
			password: postgresSource?.password ?? 'voxr',
			ssl: postgresSource?.ssl ?? false,
			sslCa: postgresSource?.ssl_ca ?? '',
			maxConnections: postgresSource?.max_connections ?? 20,
			kvTable: postgresSource?.kv_table ?? 'voxr_kv',
		...
		push: {
			...
			apns: {
				...
				apps: mapPushProviderApps(master.integrations.push.apns.apps, 'VOXR_PUSH_APNS_APPS'),
			},
			fcm: {
				...
				apps: mapPushProviderApps(master.integrations.push.fcm.apps, 'VOXR_PUSH_FCM_APPS'),
			},
		},
		worker: {
