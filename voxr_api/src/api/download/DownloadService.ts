...
import type {
	...
	DesktopFormat,
	DesktopPlatform,
} from '@voxr/schema/src/domains/download/DownloadSchemas';
...
const GITHUB_RELEASE_DOWNLOAD_BASE_URL = 'https://github.com/voxrapp/voxr/releases/download';
...
export class DownloadService {
	...
	async resolveGitHubDesktopRelease(key: string): Promise<GitHubDesktopReleaseResolution> {
		...
		if (
			...
			descriptor.release_tag !== `voxr-desktop-${scope.channel}@${manifest.version}`
		) {
	...
	private buildPossibleFilenames(
		...
		for (const archSuffix of this.getArchTokens(archMap[arch as 'x64' | 'arm64'])) {
			...
			if (format === 'setup') {
				filenames.add(`voxr-${channel}-${version}-${archSuffix}-setup${ext}`);
				filenames.add(`Voxr-${channel}-${version}-${archSuffix}-Setup${ext}`);
				filenames.add(`voxr-${version}-${archSuffix}-setup${ext}`);
				filenames.add(`Voxr-${version}-${archSuffix}-Setup${ext}`);
			} else if (format === 'portable') {
				filenames.add(`voxr-${channel}-${version}-portable-${archSuffix}${ext}`);
				filenames.add(`Voxr-${version}-portable-${archSuffix}${ext}`);
			} else {
				filenames.add(`voxr-${channel}-${version}-${archSuffix}${ext}`);
				filenames.add(`voxr-${version}-${archSuffix}${ext}`);
				filenames.add(`Voxr-${channel}-${version}-${archSuffix}${ext}`);
				filenames.add(`Voxr-${version}-${archSuffix}${ext}`);
			}
		}
	...
	private getModernProductNames(channel: DesktopChannel): Array<string> {
		return channel === 'canary' ? ['Voxr-Canary', 'Voxr Canary'] : ['Voxr'];
	}
